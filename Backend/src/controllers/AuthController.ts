...
export class AuthController {
  ...
  public async refresh(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const result = await this.authService.refresh(incomingRefreshToken, req);
      res.cookie('accessToken', result.accessToken, accessTokenCookieOptions);
      res.cookie('refreshToken', result.refreshToken, refreshTokenCookieOptions);
  ...
  @httpPost('/logout', authenticate)
  public async logout(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const accessToken: string | undefined = req.cookies?.accessToken;
      const userId = (req as any).userId!;
      if (accessToken) {
        await this.authService.logout(userId, accessToken, req);
      }
  ...
  public async me(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const user = await this.authService.getCurrentUser((req as any).userId!);
