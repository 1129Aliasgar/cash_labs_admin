...
import { useGateways, useCreateGateway, useUpdateGateway, useGatewayConfigs, useCreateGatewayConfig, useUpdateGatewayConfig } from '@/features/admin/hooks';
import type { Gateway, GatewayCapability, CreateGatewayPayload } from '@/features/admin/api/gatewayApi';
import type { GatewayRequestConfig, CreateConfigPayload } from '@/features/admin/api/gatewayRequestConfigApi';
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center p-20 text-center">
            ...
            <p className="text-slate-700 font-medium">Failed to load gateways. Check your permissions.</p>
          </div>
        ) : gateways.length === 0 ? (
          <div className="flex flex-col items-center justify-center p-20 text-center">
                ...
                <thead>
                  <tr className="bg-slate-50/80 border-b border-slate-200">
                    ...
                    {CAPABILITY_KEYS.map((key) => (
                      <th key={key} className="px-2 py-4 text-[10px] font-bold text-slate-500 uppercase tracking-wider text-center" title={`${CAPABILITY_LABELS[key]}: E=Enabled, C=Configured`}>
                        <span className="block truncate max-w-[4rem]">{CAPABILITY_LABELS[key]}</span>
                  ...
                  {gateways.map((g) => (
                    <tr key={g._id} className="hover:bg-slate-50/50 transition-colors">
                      <td className="px-6 py-4">
                        ...
                        {g.logo && (
                          <div className="text-xs text-slate-400 truncate max-w-[200px]">{g.logo}</div>
                        )}
                      </td>
                      {CAPABILITY_KEYS.map((key) => {
                        ...
                        return (
                          <td key={key} className="px-2 py-4 text-center">
                            <div className="flex items-center justify-center gap-0.5 text-xs">
                              <span className={cn('w-5 inline-block', en ? 'text-emerald-600 font-bold' : 'text-slate-300')} title="Enabled">{en ? '✓' : '—'}</span>
                              <span className="text-slate-300">/</span>
                              <span className={cn('w-5 inline-block', cfg ? 'text-blue-600 font-bold' : 'text-slate-300')} title="Configured">{cfg ? '✓' : '—'}</span>
                            </div>
                          </td>
                        );
                      })}
                    ...
                    return (
                      <button
                        ...
                        onClick={() =>
                          setForm({
                            ...form,
                            [key]: { ...form[key], enabled: !enabled, configured: form[key]?.configured ?? false },
                          })
                        }
                        className={cn(
...
const CONFIG_TYPES = ['refund', 'payment', 'apm', 'authorization', 'subscription', 'token', 'payout', 'payin'] as const;
...
function ConfigModal({
  ...
  const availableTypes = CONFIG_TYPES.filter((t) => !existingTypes.includes(t) || t === editingConfigType);
  ...
  const handleConfigSubmit = (e: React.FormEvent) => {
    ...
    if (editingConfigType) {
      updateConfig.mutate(
        { type: editingConfigType, data: { headers: payload.headers, bodyMapping: payload.bodyMapping } },
        { onSuccess }
      );
    } else {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden shadow-2xl flex flex-col">
        <div className="p-6 border-b border-slate-100 flex items-center justify-between flex-shrink-0">
          <div>
            <h2 className="text-xl font-bold text-slate-900">Request config — {gateway.name}</h2>
            <p className="text-sm text-slate-500 mt-0.5">Headers and body mapping per config type.</p>
          </div>
        ...
        <div className="p-6 overflow-y-auto flex-1">
          {configsLoading ? (
            ...
            <>
              ...
              {configs.length === 0 ? (
                <p className="text-slate-500 text-sm py-6 text-center">No request configs yet. Add one to define headers and body mapping.</p>
              ) : (
                <ul className="space-y-2">
                  {configs.map((c) => (
                    ...
                    >
                      <div>
                        <span className="font-semibold text-slate-900">{CONFIG_TYPE_LABELS[c.type] ?? c.type}</span>
                        <span className="text-xs text-slate-500 ml-2">
                          {Object.keys(c.headers?.static ?? {}).length + Object.keys(c.headers?.mapped ?? {}).length} header keys, {Object.keys(c.bodyMapping ?? {}).length} body mappings
                        </span>
                      </div>
                      <button
            ...
            <form onSubmit={handleConfigSubmit} className="space-y-4">
              <div>
                ...
                >
                  {availableTypes.map((t) => (
                    <option key={t} value={t}>{CONFIG_TYPE_LABELS[t] ?? t}</option>
                  ))}
                </select>
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">Headers (static) — JSON</label>
                <textarea
              ...
              <div>
                <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">Headers (mapped) — JSON</label>
                <textarea
              ...
              <div>
                <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">Body mapping — JSON</label>
                <textarea
              ...
              <div className="flex gap-3 pt-2">
                <button
                  type="button"
                  onClick={() => { setConfigFormOpen(false); setEditingConfigType(null); }}
                  className="flex-1 px-4 py-3 border border-slate-200 text-slate-600 rounded-xl font-bold hover:bg-slate-50"
                >
                  Cancel
                </button>
